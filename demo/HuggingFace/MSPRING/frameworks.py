...
from MSPRING.export import MT5EncoderTorchFile, MT5DecoderTorchFile
...
class MT5FHuggingFace(FrameworkCommand):
    def __init__(self):
        super().__init__(
            MT5ModelTRTConfig, description="Runs framework results for MT5 model."
        )
    ...
    def generate_and_download_framework(
        ...
        if not os.path.exists(pytorch_model_dir):
            ...
            model = MT5ForConditionalGeneration(tfm_config).from_pretrained(
                "/root/mspring/experiments/mt5-base-parser-amr3-20220103-233803/wandb/run-20220103_234104-1iks6dm8/files/hf_model"
            )
        ...
        t5_encoder = MT5EncoderTorchFile(model, metadata)
        t5_decoder = MT5DecoderTorchFile(model, metadata)
        ...
        torch_models = [
            NetworkModel(
                name=MT5ModelTRTConfig.NETWORK_FULL_NAME, fpath=pytorch_model_dir
            )
        ]
    ...
    def execute_inference(
        ...
        t5_model = MT5ForConditionalGeneration(config).from_pretrained(t5_torch_fpath)
