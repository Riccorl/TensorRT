...
# huggingface
from transformers import MT5Tokenizer, MT5Config, PretrainedConfig
...
class MT5OnnxEncoder(OnnxHFRunner):
...
class MT5OnnxDecoder(OnnxHFRunner):
...
class MT5ONNXRT(OnnxRTCommand):
    def __init__(self):
        super().__init__(
            MT5ModelTRTConfig,
            "Runs polygraphy results for MT5 model.",
            MT5FHuggingFace,
        )
    ...
    def run_onnxrt(
        ...
        try:
            ...
            self.t5_trt_encoder = MT5OnnxEncoder(
                lookup_onnx_table["encoder"].fpath, metadata, tfm_config
            )
            self.t5_trt_decoder = MT5OnnxDecoder(
