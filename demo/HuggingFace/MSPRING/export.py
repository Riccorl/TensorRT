...
"""
Contains logic that captures MT5 HuggingFace models into ONNX models.
Inspired by https://github.com/onnx/models/blob/master/text/machine_comprehension/t5/dependencies/MT5-export.py
"""
...
# TRT-HuggingFace
from MSPRING.MT5ModelConfig import MT5ModelTRTConfig
...
# Torch File Encoding #
class MT5DecoderTorchFile(TorchModelFile):
    class TorchModule(Module, GenerationMixin):
        """
        A simplied definition of MT5 Decoder without support for loss.
    ...
    def __init__(self, model, network_metadata):
        super().__init__(model, MT5DecoderConverter, network_metadata)


class MT5EncoderTorchFile(TorchModelFile):
    ...
    def __init__(self, model, network_metadata):
        super().__init__(model, MT5EncoderConverter, network_metadata)


# ONNX File Encoding #
class MT5EncoderONNXFile(ONNXModelFile):
    def __init__(self, model, network_metadata):
        super().__init__(model, MT5EncoderConverter, network_metadata)


class MT5DecoderONNXFile(ONNXModelFile):
    def __init__(self, model, network_metadata):
        super().__init__(model, MT5DecoderConverter, network_metadata)


# TRT Engine File Encoding #
class MT5DecoderTRTEngine(TRTEngineFile):
    DEFAULT_TRT_WORKSPACE_MB = 3072

    def __init__(self, model, network_metadata, batch_size = 1):
        super().__init__(model, MT5DecoderConverter, network_metadata, batch_size = batch_size)
    ...
    def get_dynamic_shape_profiles(self):
        max_sequence_length = MT5ModelTRTConfig.MAX_SEQUENCE_LENGTH[
...
class MT5EncoderTRTEngine(TRTEngineFile):
    DEFAULT_TRT_WORKSPACE_MB = 2048

    def __init__(self, model, network_metadata, batch_size = 1):
        super().__init__(model, MT5EncoderConverter, network_metadata, batch_size = batch_size)
...
# Converters #
class MT5DecoderConverter(ModelFileConverter):
    def __init__(self):
        super().__init__(MT5DecoderTorchFile, MT5DecoderONNXFile, MT5DecoderTRTEngine)

    def torch_to_onnx(
        self, output_fpath: str, model: Module, network_metadata: NetworkMetadata
    ):
        """
        Exports a given huggingface MT5 to decoder architecture only.
        ...
        simplified_encoder = MT5EncoderTorchFile.TorchModule(model.encoder)
        # Exports to ONNX
        decoder_with_lm_head = MT5DecoderTorchFile.TorchModule(
        ...
        inputs = MT5ModelTRTConfig.get_input_dims(network_metadata)["decoder"]
        outputs = MT5ModelTRTConfig.get_output_dims(network_metadata)["decoder"]
        ...
        if network_metadata.precision.fp16:
            G_LOGGER.debug("Clamping FP16 weights for MT5")
            move_t5_cast_op(output_fpath, output_fpath)
            clamp_weights_onnx_to_fp16_bounds(output_fpath, output_fpath)

        return MT5DecoderONNXFile(output_fpath, network_metadata)


class MT5EncoderConverter(ModelFileConverter):
    def __init__(self):
        super().__init__(MT5EncoderTorchFile, MT5EncoderONNXFile, MT5EncoderTRTEngine)
    ...
    def onnx_to_trt(
        ...
        Workaround: MT5-base and T5-large are too large and cause FP16 to overflow. Encoder should not use FP16 tactics even in FP16 mode.
    ...
    def torch_to_onnx(
        ...
        """
        Exports a given huggingface MT5 to encoder architecture only.
        Inspired by https://github.com/onnx/models/blob/master/text/machine_comprehension/t5/dependencies/MT5-export.py
        ...
        input_ids = torch.tensor([[42] * 10])
        simplified_encoder = MT5EncoderTorchFile.TorchModule(model.encoder)
        inputs = MT5ModelTRTConfig.get_input_dims(network_metadata)["encoder"]
        outputs = MT5ModelTRTConfig.get_output_dims(network_metadata)["encoder"]
        ...
        return MT5EncoderONNXFile(output_fpath, network_metadata)
