...
from MSPRING.frameworks import MT5FHuggingFace
from MSPRING.MT5ModelConfig import MT5ModelTRTConfig
from MSPRING.measurements import decoder_inference, encoder_inference, full_inference_greedy
from MSPRING.export import MT5DecoderONNXFile, MT5EncoderONNXFile
from NNDF.models import TRTEngineFile

# spring stuff
from mspring.data.tokenization import SpringTokenizer
...
class MT5TRTEncoder(TRTHFRunner):
    ...
    def __init__(
        ...
        self.max_sequence_length = MT5ModelTRTConfig.MAX_SEQUENCE_LENGTH[network_metadata.variant]

        # For MT5, we only have one profile to optimize
        assert len(trt_engine_file.get_dynamic_shape_profiles()) == 1, "MT5 should only have one dynamic shapes profile."
...
class MT5TRTDecoder(TRTHFRunner):
    ...
    def __init__(
        ...
        self.output_shapes = {
            "hidden_states": (self.batch_size, self.max_sequence_length, MT5ModelTRTConfig.VOCAB_SIZE)
        }
        self.output_types = {
            "hidden_states": torch.float32
        }
    ...
    def forward(self, input_ids, encoder_hidden_states, *args, **kwargs):
        ...
        # Get the vocab size.
        vocab_size = MT5ModelTRTConfig.VOCAB_SIZE
...
class MT5TRT(TRTInferenceCommand):
    def __init__(self):
        super().__init__(
            MT5ModelTRTConfig,
            "Runs trt results for MT5 model.",
            MT5FHuggingFace,
        )
    ...
    def execute_inference(
        ...
        tokenizer = SpringTokenizer(
            model_name=metadata.variant,
            predicates_vocab_path="/root/mspring/resources/vocab/predicates.txt",
            additions_vocab_path="/root/mspring/resources/vocab/additions.txt",
            recategorizations_vocab_path="/root/mspring/resources/vocab/recategorizations.txt",
            use_pointer_tokens=True,
            collapse_name_ops=False,
            min_predicate_frequency=5,
        )

        tokenizer.padding_side = "left"

        model_inputs = tokenizer([inference_input])[0]
        ...
        decoder_output_greedy = list(sliced(decoder_output_greedy.tolist(), 1))[0]

        # Remove the padding and end tokens.
        semantic_outputs = tokenizer.decode_amr(
            decoder_output_greedy[0]
        )
        ...
        return NetworkResult(
            ...
            median_runtime=[
                NetworkRuntime(
                    name=MT5ModelTRTConfig.NETWORK_DECODER_SEGMENT_NAME,
                    runtime=decoder_e2e_median_time,
                ),
                NetworkRuntime(
                    name=MT5ModelTRTConfig.NETWORK_ENCODER_SEGMENT_NAME,
                    runtime=encoder_e2e_median_time,
                ),
                NetworkRuntime(
                    name=MT5ModelTRTConfig.NETWORK_FULL_NAME,
    ...
    def run_trt(
        ...
        try:
            ...
            assert len(onnx_fpaths) == len(
                MT5ModelTRTConfig.NETWORK_SEGMENTS
            ), "There should only be {} exported ONNX segments in MT5 model.".format(
                len(MT5ModelTRTConfig.NETWORK_SEGMENTS)
            )

            hash_onnx_fpath = {v.name: v for v in onnx_fpaths}

            decoder_onnx_fpath = hash_onnx_fpath[
                MT5ModelTRTConfig.NETWORK_DECODER_SEGMENT_NAME
            ].fpath
            encoder_onnx_fpath = hash_onnx_fpath[
                MT5ModelTRTConfig.NETWORK_ENCODER_SEGMENT_NAME
            ].fpath

            self.t5_trt_encoder_engine = MT5EncoderONNXFile(
                encoder_onnx_fpath, metadata
            ).as_trt_engine(encoder_onnx_fpath + ".engine", batch_size=batch_size)
            self.t5_trt_decoder_engine = MT5DecoderONNXFile(
                decoder_onnx_fpath, metadata
            ).as_trt_engine(decoder_onnx_fpath + ".engine", batch_size=batch_size)
            tfm_config = MT5Config(
                use_cache=metadata.other.kv_cache,
                num_layers=MT5ModelTRTConfig.NUMBER_OF_LAYERS[metadata.variant],
            )
            self.t5_trt_encoder = MT5TRTEncoder(
                self.t5_trt_encoder_engine, metadata, tfm_config, batch_size=batch_size
            )
            self.t5_trt_decoder = MT5TRTDecoder(
